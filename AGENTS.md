...
2. leia o `PROJECT_PROMPT.md` quando a tarefa for construir o MVP inicial;
3. consulte o `SYSTEM_BLUEPRINT.md` como fonte técnica e visual do produto;
4. inspecione as imagens em `references/` quando a tarefa afetar interface ou identidade.

Em caso de conflito, siga esta ordem: solicitação atual do usuário, este arquivo, `PROJECT_PROMPT.md`, `SYSTEM_BLUEPRINT.md` e referências visuais.
...
- Arquivos em `references/` são somente leitura: não edite, renomeie, mova ou exclua.

## 4. Uso das referências visuais

Use `references/GoDocs/` para orientar:
...
Use `references/AbacatePay - Mintlify/` para orientar:
...
- Metadados devem seguir o contrato definido no `SYSTEM_BLUEPRINT.md`.
